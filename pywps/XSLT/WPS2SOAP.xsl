...
<!-- License: GPL -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fn="http://pywps.wald.intevation.org/functions" version="1.0">
  <xsl:template match="/">
    ...
    <xsl:variable name="identifierSOAP">
      <xsl:choose>
        <xsl:when test="$async">
          <xsl:value-of select="concat('ExecuteProcessAsync_',$identifier,'Response')"/>
        </xsl:when>
        <xsl:otherwise>
      ...
      <xsl:choose>
        <xsl:when test="$async">
          ...
          <xsl:element name="statusURLResult">
            <xsl:value-of select="//*[local-name()='ExecuteResponse']/@statusLocation"/>
          </xsl:element>
        </xsl:when>
        <xsl:otherwise>
