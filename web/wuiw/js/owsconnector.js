...
function owsConnector(console) {
 ...
 owsConnector.prototype.addServer = function (title, url){
	 this.aServer.push([title,url]);
 };
 
 
 owsConnector.prototype.drawInitForm = function (objId){
...
//	select.owsConnector=this;
 ...
 owsConnector.prototype.connect2server = function (){
	 var self = owsConnector;
 ...
 owsConnector.prototype.parseCapabilities = function (xml){
 ...
 owsConnector.prototype.drawLayersForm = function(xml){
	 	var self= owsConnector;
  ...
  owsConnector.prototype.getWmsLayer = function (){
	  var self = owsConnector;
    ...
    var myURL = connector + 'owsURL='+myurlencode(this.baseURL);
    //myURL = this.owsConnector.addRequestParameter(myURL, 'com', "&com=getCapabilities" );
 ...
 owsConnector.prototype.getCoords =  function (e){
	  var self = owsConnector;
...
owsConnector.prototype.pix2geo =  function (pX,pY){
	  var self = owsConnector;
...
//trasform coords from image pixel space to map geo space
owsConnector.prototype.waitStart =  function (){
	self = owsConnector;
...
//trasform coords from image pixel space to map geo space
owsConnector.prototype.waitEnd =  function (){
	self = owsConnector;
	var wait = getRawObject('wait');
	if( wait)wait.parentNode.removeChild(wait);
}
