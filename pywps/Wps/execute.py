...
class Execute:
    ...
    def __init__(self, serverSettings, grassSettings, process,formvalues,method):
        ...
        try:
            ...
            sys.stderr.write( "PyWPS Warning: Execute.execute failed with KeyError: %s\n" % (e) )
        except AttributeError,e :
            sys.stderr.write( "PyWPS Warning: Execute.execute failed with AttributeError: %s\n" % (e) )
            ...
            if 'store' in self.formvalues.keys() and\
                ...
                if 'ComplexValueReference' in procOutput.keys():
                        ...
                        try:
                            ...
                            self.cleanTempDirs()
                            raise ServerError("IOError: %s" % what)
    ...
    def cleanTempDirs(self,directory=None):
        """
    ...
    def mktempdir(self,directory=None):
        ...
        try:
            ...
            raise ServerError("IOError, OSError: tempdir not created - %s"  % what)
        ...
        for input in self.process.Inputs:
            try:
                ...
                if not input.has_key('value') or input['value'] == None:
                    raise MissingParameterValue(input['Identifier'])
        #
